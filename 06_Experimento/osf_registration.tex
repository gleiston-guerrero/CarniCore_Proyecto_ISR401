...
\url{https://osf.io/wud69}. Ese identificador corresponde al \textbf{proyecto}
de OSF del que partió el estudio (público, al igual que el registro), no al
\textbf{registro} (\textit{registration}) propiamente dicho. La distinción
importa porque un proyecto de OSF es un espacio de trabajo editable en
cualquier momento, mientras que una \textit{registration} finalizada queda
congelada de forma permanente con un sello \textit{Date Registered} que ya no
puede alterarse: es ese carácter inmutable, y no la visibilidad, lo que la
hace válida como comprobante de precedencia temporal. \texttt{README.md} y
\texttt{CHANGELOG.md} ya documentaban esta corrección --el identificador del
registro es \url{https://osf.io/yp7t3}-- y la propagaron a los demás archivos
del repositorio, pero este documento, que es precisamente el comprobante,
había quedado sin corregir. Esta versión 2.0 lo corrige y ajusta los campos
que dependían del objeto equivocado.
\end{notaerratum}
...
Naturaleza del objeto & \textit{Registration} de OSF (no el proyecto
\texttt{osf.io/wud69} del que partió el estudio: ambos son públicos, pero solo
la \textit{registration} queda congelada de forma permanente con un sello
\textit{Date Registered} inalterable, que es lo que exige el gatekeeper de
...
Tanto el proyecto \texttt{wud69} como el registro \texttt{yp7t3} son públicos,
así que ambos enlaces son visitables por cualquier tercero --incluido el
tribunal evaluador-- sin sesión autenticada. La diferencia relevante no es la
visibilidad sino la mutabilidad: un proyecto de OSF sigue siendo editable
después de creado, por lo que su fecha de creación no certifica que su
contenido no haya cambiado desde entonces. Una \textit{registration}
finalizada, en cambio, queda congelada de forma permanente; el sello
\textit{Date Registered} certifica el estado exacto del plan de análisis en
ese instante, y es precisamente esa propiedad --no la mera existencia de una
fecha pública-- la que exige el gatekeeper de precedencia temporal. Por eso el
comprobante debe anclarse en \texttt{yp7t3} y no en \texttt{wud69}, aunque
ambos sean consultables por un tercero.
...
proyecto \texttt{wud69}. Revisando el contenido real de las imágenes, eso solo
es cierto para la primera: es un recorte del campo \textit{Associated Project}
del panel lateral del propio registro \texttt{yp7t3}, que enlaza a su proyecto
de origen, \texttt{osf.io/wud69} (público, igual que el registro). La segunda
captura es en realidad el \textbf{Overview del propio registro} \texttt{yp7t3}
(pestaña \textit{Registry Details -> Overview}), no del proyecto: muestra el
contenido íntegro del preregistro y, en su panel lateral, \textit{Registration
Type: OSF Preregistration}, \textit{Registry: OSF Registries} y los sellos
\textit{Date Created} / \textit{Date Registered} -- exactamente los que se
citan en la tabla de esta página. Se corrigen aquí las leyendas de ambas
figuras.
\end{notaerratum}
...
\begin{figure}[H]
    ...
    \texttt{osf.io/yp7t3}, que enlaza a su proyecto de origen
    (\texttt{osf.io/wud69}, público).}
\end{figure}
